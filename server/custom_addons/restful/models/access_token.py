...
class APIAccessToken(models.Model):
    ...
    def find_one_or_create_token(self, user_id=None, create=False):
        ...
        if access_token:
            _logger.info("@@@@@@@@@@@@@@@@@@@ token exist")
            access_token = access_token[0]
            if access_token.has_expired():
                _logger.info("@@@@@@@@@@@@@@@@@@@ token expired")
                access_token = None
        if not access_token and create:
            _logger.info("@@@@@@@@@@@@@@@@@@@ token not exist. Create new")
            expires = datetime.now() + timedelta(
                seconds=int(self.env['ir.config_parameter'].sudo().get_param(expires_in)) # self.env.ref(expires_in).sudo().value
            )
            vals = {
                "user_id": user_id,
                "scope": "userinfo",
